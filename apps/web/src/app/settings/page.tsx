...
export default function SettingsPage() {
  ...
  async function connect(service: string) {
    const response = await authFetch(`/api/integrations/google/connect?service=${service}`);
  ...
  async function disconnect(service: string) {
    await authFetch(`/api/integrations/google/disconnect?service=${service}`, {
      method: "POST", });
  ...
  async function createDriveRoot() {
    ...
    const response = await authFetch(`/api/integrations/google/drive/create-root`, {
      method: "POST", });
    const data = await response.json();
    if (!response.ok) {
  ...
  async function syncFixtureGmail() {
    const response = await authFetch(`/api/integrations/gmail/sync-fixture`, {
      method: "POST", });
    setMessage(JSON.stringify(await response.json()));
  }
